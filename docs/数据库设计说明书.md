...
| 版本 | v1.0，对应《校园跑腿交易平台需求分析》v1.1 第一周交付范围 |
| 表数量 | 5 张表 + 2 个视图 |
...
| `carol` | `carol123` | 普通用户 | 备用，演示管理员的用户列表 |

演示数据已覆盖全部状态：待审核、已通过、已驳回、已接取、已完成、已删除、已售出。**前端不用等后端接口，直接读这个库就能调页面。**
...
需求文档 §4.2 说任务状态是「待审核 → 已发布」，§6 又说是「待接取-已接取-进行中-已送达-完成」，两处对不上。原因是它们描述的是**两个互相独立的维度**：

| 维度 | 字段 | 回答的问题 | 谁能改 |
|---|---|---|---|
| 审核 | `audit_status` | 这条内容能不能被普通用户看见 | 管理员 |
| 业务 | `status` | 这一单交易走到哪一步了 | 发布者 / 接取方 |

所以一条新任务落库时是 `audit_status='pending'` + `status='open'`：业务上它是"待接取"，但审核上它还没过，所以列表里看不见。管理员点通过只改 `audit_status`，任务立刻出现在列表且可被接取。

**塞进一个字段会怎样**：假设只有一个 `status`，那么"已接取的任务被管理员驳回"这种情况就没法表达——你得在"已接取"和"已驳回"里二选一，丢掉另一个信息。拆成两个字段就没这个问题。

删除同理，用独立的 `is_deleted` 而不是 `status='deleted'`：管理员删掉一条**已完成**的任务后，`status` 仍然是 `completed`，历史记录不会被破坏，符合需求 §4.2「数据库保留删除状态」。
...
| `role` | TEXT | CHECK, 默认 `user` | `user` / `admin` |
| `status` | TEXT | CHECK, 默认 `active` | `active` / `banned`，第一周不用，预留封禁 |
| `created_at` / `updated_at` | TEXT | NOT NULL | 见下方时间格式约定 |

> 管理员账号由 `init_db.py` 预置，注册接口**必须**写死 `role='user'`，不能接受前端传入的 role，否则任何人都能注册成管理员。
...
| `audit_status` | TEXT | CHECK, 默认 `pending` | `pending`/`approved`/`rejected` |
| `audit_remark` | TEXT | 可空 | 驳回理由，前端在个人空间展示给发布者 |
| `status` | TEXT | CHECK, 默认 `open` | `open`/`accepted`/`delivered`/`completed`（+1 个预留） |
| `is_deleted` | INTEGER | CHECK ∈ (0,1) | 管理员软删除标记 |
| `deleted_by` | INTEGER | FK→user | 哪个管理员删的 |
| `deleted_at` | TEXT | 可空 | 删除时间 |
...
| `user.role` | `user` | 普通用户 |
| | `admin` | 管理员 |
| `task.audit_status`<br>`product.audit_status` | `pending` | 待审核 |
| | `approved` | 已通过 |
| | `rejected` | 已驳回 |
...
```
                    ┌──────────────────────────────────────────┐
   用户发布          │  audit_status = pending, status = open   │  列表不可见
                    └────────────────┬─────────────────────────┘
                                     │ 管理员审核
                      ┌──────────────┴───────────────┐
                      ▼                              ▼
        audit_status = approved            audit_status = rejected
        （出现在列表，可被接取）              （仅发布者在个人空间可见，
                      │                       附 audit_remark 驳回理由）
                      │ 他人点击接取
              ...
              status = delivered，task_order.delivered_at = 当前时间
                      │ 发布者点「确认收到」（管理员可强制确认兜底）
                      ▼
              status = completed，task_order.status = completed，finished_at = 当前时间

   任意阶段：管理员删除 → is_deleted = 1（列表消失，数据保留）
```

**完成需要双方确认**：接单方单方面点不了「完成」，只能点到「已送达」；发布者确认收到才算真正完成。这样双方都不能单方面把一单说成完成的。

发布者一直不确认怎么办？管理员可以强制标记完成（需求 §4.2 给了管理员这个权限），当作兜底。

### 二手商品

```
   发布 → pending / on_sale  ──审核通过──▶ approved / on_sale（可购买）
                             ──审核驳回──▶ rejected（前台不可见）
...
-- 管理员：待审核列表
SELECT * FROM task    WHERE audit_status = 'pending' ORDER BY created_at ASC;
SELECT * FROM product WHERE audit_status = 'pending' ORDER BY created_at ASC;

-- 管理员：审核通过 / 驳回
UPDATE task SET audit_status = 'approved', updated_at = datetime('now','localtime') WHERE id = ?;
UPDATE task SET audit_status = 'rejected', audit_remark = ?, updated_at = datetime('now','localtime') WHERE id = ?;

-- 管理员：软删除任务
UPDATE task SET is_deleted = 1, deleted_by = ?, deleted_at = datetime('now','localtime') WHERE id = ?;
...
另外验证了：待审核 / 已驳回 / 已删除的内容确实不会出现在前台视图里，且软删除后原数据仍完整保留在库中。
